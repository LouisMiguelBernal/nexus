...
class Gemma4:
    ...
    async def status(self, ttl: float = 10.0) -> Dict:
        ...
        it reported a healthy Gemma 4 while Ollama was refusing connections on
        11434. This returns what is actually true. Cached for ``ttl`` seconds
        because the risk tab polls health on an interval."""
        now = time.time()
        if self._status_cache and (now - self._status_cache_at) < ttl:
            return self._status_cache

        installed = await self._fetch_installed()
        ...
        self._status_cache = {
            "state": state,
            "ok": state.startswith("ready"),
            "model": self.model,
            "server_reachable": bool(self.server_reachable),
            "model_installed": model_installed,
            "model_resident": is_resident,
    ...
    async def _call_once(
        ...
        payload = {
            ...
            # Without this Ollama evicts the model after its 5m default, so a
            # user who clicks twice ten minutes apart pays the ~85s cold load
            # twice. Keeping it resident makes every repeat click warm.
            "keep_alive": self.keep_alive,
        }
        if system:
    ...
    async def warmup(self) -> bool:
        """Load the primary model into memory ahead of the first user click.

        Ollama loads a model on its first generate call; for gemma4:e4b that is
        ~85s of dead air. Issuing a 1-token request at startup moves that cost
        ...
        if err:
            ...
            return False
        logger.info(f"Gemma4 warmup complete model={self.model} (resident {self.keep_alive})")
        return True
