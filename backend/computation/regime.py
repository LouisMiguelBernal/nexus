...
# ---------------------------------------------------------------------------
# Cross-asset conditioning hook
#
# Registered once at app startup (main.py) and left None everywhere else, so a
# backtest keeps a classifier that is a pure function of the klines handed to
# it - a backtest must never silently depend on today's live macro board.
#
# It lives here rather than at each call site because there are four of them
# (alpha engine, matrix router, /api/alpha, /api/brief). Wrapping each by hand
# is how the reported regime and the regime the weights were selected from
# drift apart. Conditioning inside classify() makes that impossible.
#
# The conditioner may only return a label already present in REGIMES; callers
# fall back to their own defaults if that contract is broken, so a bad hook
...
def set_conditioner(fn) -> None:
    """Install (or clear, with None) the cross-asset regime conditioner."""
    global _CONDITIONER
    _CONDITIONER = fn
...
def _condition(result: dict) -> dict:
    if _CONDITIONER is None:
        return result
    try:
        conditioned = _CONDITIONER(result)
        return conditioned if isinstance(conditioned, dict) else result
    except Exception:
...
class RegimeClassifier:
    ...
    def classify(
        ...
        """
        if len(closes) < 20:
            return _condition({"regime": "insufficient_data", "confidence": 0})
