...
def get_last_brief() -> dict | None:
    conn = get_connection()
    row = conn.execute("SELECT * FROM briefs ORDER BY generated_at DESC LIMIT 1").fetchone()
    return dict(row) if row else None


def get_briefs(limit: int = 10) -> list[dict]:
    conn = get_connection()
    rows = conn.execute("SELECT * FROM briefs ORDER BY generated_at DESC LIMIT ?", (limit,)).fetchall()
