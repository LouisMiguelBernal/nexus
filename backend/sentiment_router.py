...
async def _pub_get(url: str, params: dict | None = None) -> Any:
    ...
    async with httpx.AsyncClient(timeout=10) as c:
        r = await c.get(url, params=params or {}, headers={"User-Agent": "NexusTerminal/0.3"})
        r.raise_for_status()
        return r.json()
...
def _score_volatility(price_chg_24h: float, high_24h: float, low_24h: float, close: float) -> float:
    """
...
def _score_momentum(price_chg_24h: float, price_chg_7d: float, volume_ratio: float) -> float:
    """
...
def _ai_interpret_fng(
    score: int, label: str, price_chg: float, funding: float, ls_ratio: float, oi_chg: float
) -> str:
    """
...
async def _compute_fng() -> dict[str, Any]:
    ...
    # Parallel fetch
    ticker_task = _pub_get(f"{BINANCE_FAPI}/fapi/v1/ticker/24hr", {"symbol": "BTCUSDT"})
    funding_task = _pub_get(f"{BINANCE_FAPI}/fapi/v1/premiumIndex", {"symbol": "BTCUSDT"})
    ls_task = _pub_get(
        f"{BINANCE_FAPI}/futures/data/globalLongShortAccountRatio",
        {"symbol": "BTCUSDT", "period": "1h", "limit": 2},
    )
    oi_task = _pub_get(
        f"{BINANCE_FAPI}/futures/data/openInterestHist", {"symbol": "BTCUSDT", "period": "1h", "limit": 25}
    )

    results = await asyncio.gather(ticker_task, funding_task, ls_task, oi_task, return_exceptions=True)
    ...
    # Weighted composite
    composite = vol_score * 0.25 + mom_score * 0.25 + fund_score * 0.20 + ls_score * 0.20 + oi_score * 0.10
    score = int(round(_clamp(composite)))
    label, color = _label_fng(score)
    interpretation = _ai_interpret_fng(score, label, price_chg, funding_pct, ls_ratio, oi_chg_pct)
...
def _classify_signal(
    top_ratio: float, top_delta_6h: float, oi_vel: float, price_chg: float, funding: float
    ...
    # Label
    ratio_pct = top_ratio / (top_ratio + 1) * 100  # noqa: F841
    if top_ratio >= 1.5:
        label = "STRONG LONG"
    elif top_ratio >= 1.15:
        label = "LONG BIAS"
...
def _ai_insight(
    ...
    lines.append(
        f"Top traders are {strength} positioned {direction} ({top_ratio:.2f} ratio, Δ6h={top_delta_6h:+.3f})."
    )
    ...
    return " ".join(l for l in lines if l)  # noqa: E741
...
async def _compute_smart_money(symbol: str = "BTCUSDT") -> dict[str, Any]:
    ...
    global_task = _pub_get(
        f"{BINANCE_FAPI}/futures/data/globalLongShortAccountRatio",
        {"symbol": symbol, "period": "1h", "limit": 24},
    )
    oi_task = _pub_get(
        f"{BINANCE_FAPI}/futures/data/openInterestHist", {"symbol": symbol, "period": "1h", "limit": 48}
    )
    mark_task = _pub_get(f"{BINANCE_FAPI}/fapi/v1/premiumIndex", {"symbol": symbol})
    ticker_task = _pub_get(f"{BINANCE_FAPI}/fapi/v1/ticker/24hr", {"symbol": symbol})

    results = await asyncio.gather(
        top_task, global_task, oi_task, mark_task, ticker_task, return_exceptions=True
    )
    ...
    if top_ls and isinstance(top_ls, list) and len(top_ls) >= 1:
        ...
        if top_delta_6h > 0.04:
            top_trend = "rising"
        elif top_delta_6h < -0.04:
            top_trend = "falling"
    ...
    fund_label = "neutral"
    if funding > 0.10:
        fund_label = "extreme_longs"
    elif funding > 0.04:
        fund_label = "elevated_longs"
    elif funding < -0.10:
        fund_label = "extreme_shorts"
    elif funding < -0.04:
        fund_label = "elevated_shorts"
    ...
    insight = _ai_insight(top_ratio_now, top_delta_6h, divergence_type, sig_type, oi_vel, funding, price_chg)
    ...
    if top_trend == "rising":
        score += 12
    if top_trend == "falling":
        score -= 12
    if divergence_type == "smart_money_long_retail_short":
        score += 18
    if divergence_type == "smart_money_short_retail_long":
        score -= 18
...
async def get_fng():
    ...
    try:
        ...
        return {
            ...
            "color": "#f1c40f",
            "error": str(e),
            "ai_interpretation": "Data unavailable.",
        }
...
async def debug():
    ...
    return {
        ...
        "sm_signal": _sm_cache.get(list(_sm_cache.keys())[0], {}).get("signal", {}).get("label")
        if _sm_cache
        else None,
