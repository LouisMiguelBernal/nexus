...
# --- Cross-asset context + global-event intelligence ---
...
# Cross-asset + global-event snapshot. Written only by world_poller.run(),
# read by the /api/world routes and by the regime conditioning in /api/alpha.
world_state = world_poller.WorldState()
...
async def lifespan(app: FastAPI):
    ...
    # Cross-asset board + FRED macro + global-event risk. Owns freshness for
    # every /api/world route and publishes the geo score into the macro gate.
    asyncio.create_task(world_poller.run(world_state, macro_gate))

    # Condition every regime read on the cross-asset board. Registered on the
    # classifier itself, so the alpha engine, the matrix router and the
    # /api/alpha and /api/brief routes all read the same conditioned label -
    # wrapping call sites individually is how the reported regime and the regime
    # the weights were selected from drift apart.
    #
    # The conditioner can only tip a low-confidence label between the five
    # existing regimes, or shrink confidence on a macro/microstructure
    # divergence. It never introduces a label WEIGHTS_BY_REGIME does not key on,
    ...
    llm_status = await gemma4.status(ttl=0)
    if not llm_status["server_reachable"]:
        logger.error(
            "OLLAMA UNREACHABLE at %s - AI briefs WILL fail. Start it with: ollama serve",
            gemma4.endpoint,
        )
    ...
    if llm_status["missing_fallbacks"]:
        logger.warning(
            ...
            "If the primary OOMs there is nothing to fall back to. "
            "Pull one with: ollama pull %s",
            ", ".join(llm_status["missing_fallbacks"]),
            llm_status["missing_fallbacks"][0],
        )
    logger.info(f"BloFin (paper): {'connected' if blofin.connected else 'not configured'}")

    # Pre-warm the default-view response caches so the FIRST UI load is instant
    # instead of paying every cold fetch (klines/alpha/derivatives/strip) at
    # once. Runs in the background; never blocks startup.
    async def _warm_default_caches() -> None:
        try:
            await asyncio.sleep(4)  # let the WS klines seed + server start serving
            sym = DEFAULT_SYMBOLS[0]
            await asyncio.gather(
                get_klines(sym, "1h", 500),
                get_crypto_strip(),
                get_alpha(sym),
                get_derivatives_strip(sym, "1h", 500),
                return_exceptions=True,
            )
            # Matrix lives on a mounted router (closure) - warm it (and its
    ...
    # Load gemma4:e4b into memory now rather than on the user's first click.
    # Cold load is ~85s on this box; doing it lazily meant GENERATE AI BRIEF
    # blew its request budget and returned an empty brief.
    async def _warm_llm() -> None:
        if not gemma_ok:
            logger.info("Gemma 4 warmup skipped - model not installed")
            return
        try:
            await asyncio.sleep(8)  # let the data caches warm first
            await gemma4.warmup()
            # FinBERT is the other lazily-loaded model in the brief path: its
            # first score_batch pulls weights onto CUDA and cost ~95s on the
            # user's first click. Load it off the critical path too. It is
            # sync/CPU-blocking, so keep it off the event loop.
...
async def health():
    return {
        ...
        **build_info(),
        ...
        # Live probe. The bare model-name echo above reported a healthy Gemma 4
        # while Ollama was refusing connections, which is how a dead inference
        # server stayed invisible until a user clicked GENERATE AI BRIEF.
        "llm": await gemma4.status(),
...
_STRIP_TTL_SEC = 20.0  # 24h stats move slowly; the strip does not need 5s freshness
...
async def get_crypto_strip():
    ...
    async with _strip_lock:
        ...
        if hit and time.time() - hit[0] < _STRIP_TTL_SEC:
            return hit[1]
        # Fetch ONLY the 10 watchlist symbols, concurrently (weight 1 each = 10),
        # instead of the all-pairs ticker (~270KB, weight 40, 418-prone).
        results = await asyncio.gather(
            *(_afetch(f"/fapi/v1/ticker/24hr?symbol={s}") for s in STRIP_SYMBOLS),
            return_exceptions=True,
        )
        quotes = []
        ...
        if quotes:
            ...
            return payload
        # Fetch failed (rate-limit / network) - serve last good data if we have it
        # so the ticker never blanks out.
        return hit[1] if hit else payload
...
async def generate_ai_brief():
    ...
    _t_fetch = time.time()
    funding_r, oi_r, news_r = await asyncio.gather(
        funding_trackers[sym].fetch_all() if sym in funding_trackers else _empty(),
        oi_trackers[sym].fetch_all() if sym in oi_trackers else _empty(),
        news_feed.fetch_all(),
        return_exceptions=True,
    )
    for _label, _res in (("funding", funding_r), ("oi", oi_r), ("news", news_r)):
        if isinstance(_res, BaseException):
            logger.warning("ai/brief %s fetch failed: %s", _label, _res)
    funding = funding_r if isinstance(funding_r, dict) else {}
...
# derivatives streams. 12s dedupes rapid re-requests (tab/interval switches)
# without a visible lag, and was previously shorter than the ~1.7s fetch.
