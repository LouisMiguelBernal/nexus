...
class PortfolioMarginClient:
    ...
    def __init__(self):
        self._is_pm_account: bool | None = None
    ...
    async def get_leverage_brackets(self, symbol: str) -> list | None:
    ...
    async def get_margin_summary(self) -> dict:
        ...
        for p in positions:
            notional = abs(
                float(p.get("notional", p.get("positionAmt", 0)))
                * float(p.get("markPrice", p.get("entryPrice", 0)))
            )
