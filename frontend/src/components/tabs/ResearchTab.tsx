  ...
  const generateAiBrief = async () => {
    ...
    try {
      const r = await fetch(`${api}/api/ai/brief`, {
        method: "POST",
        // Backend allows 300s per Ollama call and makes two (brief + news),
        // so the client must not give up first.
      ...
      const stored: StoredBrief = {
        ...
        // The backend returns HTTP 200 with an empty brief + brief_error when
        // the local LLM fails. Hard-coding null here swallowed that and left
        // the card showing "No AI synthesis yet" with no explanation.
        error: (d.brief_error as string | null)
            ?? (d.news_error as string | null)
            ?? (!r.ok ? `Backend returned HTTP ${r.status}` : null),
      };
      setAiBrief(stored);
